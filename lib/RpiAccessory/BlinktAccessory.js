...
class BlinktAccessory extends GpioAccessory {
  constructor (rpiAccessory, device) {
    ...
    this.blinkt = device.device === 'p9813'
    ...
    this.blinkt
      ...
      .on('led', (id, led) => {
        if (device.device === 'p9813') {
          this.debug(
            'led %d: send rgb: {%d, %d, %d}', id, led[3], led[2], led[1]
          )
        } else {
          this.debug(
            'led %d: send bri: %d, rgb: {%d, %d, %d}', id,
            led[0] & 0x1F, led[3], led[2], led[1]
          )
        }
      })
      .on('request', (request) => {
