        ...
        for (const j in config.devices) {
          ...
          switch (device.device) {
            ...
            case 'rocker':
              ...
              result.pull = 'up'
              parser.enumKey('pull')
              for (const pud in GpioClient.pudValues) {
  ...
  async checkDevice (host) {
    ...
    if (host.port === 8888) {
      if (GpioClient.Pigpio == null) {
        await import('./GpioClient/PigpioClient.js')
      }
        ...
        try {
          switch (device.device) {
            ...
            case 'p9813':
              ...
              break
            case 'button':
              if (pi.port !== 8888) throw new Error(`${device.device} device not yet supported through gpiod`)
            ...
            case 'doorbell':
              ...
              break
            case 'fan':
              if (pi.port !== 8888) throw new Error('not yet supported through gpiod')
