      ...
      for (const i in this.config.hosts) {
        ...
        for (const j in config.devices) {
          ...
          switch (device.device) {
            ...
            case 'ds18b20':
            case 'servo':
              if (device.device !== 'ds18b20') {
                mandatoryKeys.push('gpio')
                parser.intKey('gpio', 0, 31)
              }
              parser.stringKey('sensorId')
              break
            case 'fanshim':
              break
            default:
          ...
          parser.parse(device)
          if (
            result.device === 'ds18b20' &&
            result.sensorId != null &&
            (
              typeof result.sensorId !== 'string' ||
              !RpiAccessory.DS18B20_SENSOR_ID_PATTERN.test(result.sensorId)
            )
          ) {
            this.warn(
              'config.json: hosts[%d]: devices[%d]: sensorId: expected DS18B20 ID (28-...)',
              i, j
            )
            continue
          }
        ...
        if (config.w1) {
          validDevices.push({
            device: 'ds18b20',
            name: '1-Wire Temperature Sensor'
          })
        }
        ...
        try {
          switch (device.device) {
            ...
            case 'dht':
              ...
              break
            case 'ds18b20':
              await rpiAccessory.addDs18b20(device)
              break
            case 'doorbell':
