...
class RpiInfo extends events.EventEmitter {
  ...
  /** Read `/proc/cpuinfo` and extract the serial number and hardware revision
    ...
    * when `/proc/cpuinfo` or `/proc/1/sched` cannot be read, or
    * when `vcgencmd` cannot be run.
    */
  async getCpuInfo () {
    if (os.platform() !== 'linux' || !['arm', 'arm64'].includes(os.arch())) {
    ...
    await this.vcgencmd('measure_temp') // Throws if vcgencmd isn't available.
    return cpuInfo
  }

  static noPowerLed (model) {
    return ['A', 'B', 'Zero', 'Zero W', 'Zero 2 W'].includes(model)
  }
  ...
  async getPowerLedState (noPowerLed) {
    if (!noPowerLed) {
      try {
        ...
        const powerLed = await fs.readFile(RpiInfo.powerLed, 'utf8')
        return powerLed
      } catch (error) { }
    }
    return '255'
