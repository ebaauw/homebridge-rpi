...
class RpiInfo {
  /** Read `/proc/cpuinfo` and extract the serial number and hardware revision
    ...
    * @throws `Error` - When not running on a Raspberry Pi
    * or when `/proc/1/sched` or `/proc/cpuinfo` cannot be read.
    */
  static async getCpuInfo () {
    if (os.platform() !== 'linux' || os.arch() !== 'arm') {
      throw new Error(`${os.arch()} running ${os.platform()}`)
    }
    const sched = await fs.readFile('/proc/1/sched')
    if (!sched.startsWith('systemd (1, #treads: 1)')) {
