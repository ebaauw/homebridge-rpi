...
class PigpioClient extends GpioClient {
  ...
  */
  bufferSize (params) {
    return MESSAGE_SIZE + (Buffer.byteLength(params.p3) ?? 0)
  }
  ...
  /** Decode the contents of this._data.
    ...
    */
  decode () {
    const cmd = this._data.readUInt32LE(0)
    const result = {
      status: unsignedCommands.includes(cmd)
        ? this._data.readUInt32LE(12)
        : this._data.readInt32LE(12)
    }
  ...
  gpioOutput (gpio) {
    return {
      ...
      params: { p1: gpio, p2: modeValues.OUTPUT, p3: '' }
    }
  }
  ...
  async readFile (filename) {
    ...
    this._fileHandle = (await this.command(commands.FO, 1, 0, filename)).status
    ...
    do {
      response = await this.command(
        commands.FR, this._fileHandle, this._params.blockSize
      )
      s += response.string
    } while (response.buffer.length === this._params.blockSize)
    await this.command(commands.FC, { p1: this._fileHandle })
    delete this._fileHandle
    return s
  }
  ...
  async writeFile (filename, text) {
    ...
    this._fileHandle = (await this.command(commands.FO, 2, 0, filename)).status
    if (this._fileHandle !== 0) {
      this.emit('warning', new Error(`got file handle ${this._fileHandle}`))
    }
  ...
  async listen (mask = 0x0FFFFFFC) {
    ...
    this._dataSocket
      .on('data', (data) => { this._onDataData(data) })
    ...
    this._dataSocket.write(request) // TODO async
    let p
    try {
    ...
    this._mask = mask
    await this.command(commands.NB, this._dataHandle, mask >>> 0)
    ...
    this._checkNotification({ seqno: 0, flags: 0x0000, tick, map })
  }
  ...
  _onDataData (data) {
    ...
    while (data.length >= 12) {
      if (
        ...
        this.emit(commands.NOIB, { status: res })
