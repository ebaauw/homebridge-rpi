...
class PigpioClient extends GpioClient {
  ...
  async listen () {
    ...
    await this.command(commands.NB, this.dataHandle, this.mask >>> 0)
  ...
  async writePwm (gpio, dutyCycle) {
    await this.command(this.commands.PWM, gpio, Math.round(dutyCycle * 2.55))
  }

  async writeServo (gpio, pulseWidth) {
    await this.command(this.commands.SERVO, gpio, pulseWidth)
  }
