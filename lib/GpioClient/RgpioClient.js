...
class RgpioClient extends GpioClient {
  ...
  async listen (mask = 0xFFFFFFFC) {
    ...
    for (let gpio = 0; gpio < 32; gpio++) {
      ...
      await this.command(commands.GSAX, { longs: [this.gpioChipHandle, 0, 3, gpio, this.dataHandle] })
      {
        const result = await this.command(commands.GR, { longs: [this.gpioChipHandle, gpio] })
        const value = result.status === 1
        if (this.values[gpio] !== value) {
          this.emit('warning', `gpio ${gpio}: value changed from ${this.values[gpio]} to ${value}`)
          await this.command(this.commands.GW, { longs: [this.gpioChipHandle, gpio, this.values[gpio] ? 1 : 0] })
        }
      }
    }
  ...
  */
  ...
  async setOutput (gpio, value = 0) {
    ...
    await this.command(this.commands.GSOX, { longs: [this.gpioChipHandle, 0, gpio, value] })
  }

  async dhtPoll (gpio) {
    await this.setOutput(gpio, 0)
    await timeout(18)
    await this.command(this.commands.GSAX, { longs: [this.gpioChipHandle, 0, 1, gpio, this.dataHandle] })
  }
