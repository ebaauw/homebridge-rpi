...
class RgpioClient extends GpioClient {
  ...
  */
  ...
  async setPwm (gpio) {
    await this.command(this.commands.GSOX, { longs: [this.gpioChipHandle, gpioModes.none, gpio, 0] })
  }
  ...
  async writeServo (gpio, pulseWidth) {
    await this.command(this.commands.SX, {
      longs: [this.gpioChipHandle, gpio, pulseWidth, 200, 0, 0]
    })
  }
