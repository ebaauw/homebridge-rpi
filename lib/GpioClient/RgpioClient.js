...
class RgpioClient extends GpioClient {
  ...
  async connect () {
    ...
    const result = await this.command(commands.GIC, { longs: [this._gpioChipHandle] })
  ...
  */
  ...
  decode () {
    ...
    if (size > 0) {
      const buffer = this._data.subarray(MESSAGE_SIZE, len)
      switch (cmd) {
        case commands.FR:
  ...
  async listen (mask = 0xFFFFFFFC) {
    ...
    await this.write(request, this._dataSocket)
    let p
    try {
    ...
    this._dataHandle = response.status
    if (this._dataHandle !== 0) {
      this.emit('warning', new Error(`got data handle ${this._dataHandle}`))
    }
    this._dataSocket.on('data', (data) => { this.onDataData(data) })

    await this.command(commands.GIL, { longs: [this._gpioChipHandle, 18] })
    await this.command(commands.GSOX, { longs: [this._gpioChipHandle, 0, 18, 0] })
    await this.command(commands.GIL, { longs: [this._gpioChipHandle, 18] })
    ...
    for (let gpio = 0; gpio < 32; gpio++) {
      ...
      await this.command(commands.GSAX, { longs: [this._gpioChipHandle, 135, 3, gpio, this._dataHandle] })
    }
    await this.command(commands.GIL, { longs: [this._gpioChipHandle, 18] })
  ...
  onDataData (buffer) {
    ...
    while (this._data.length >= this.MESSAGE_SIZE) {
      const payload = {
        tick: this._data.readBigUInt64LE(0), // nanoseconds since boot?
        chip: this._data.readUInt8(8),
        gpio: this._data.readUInt8(9),
        level: this._data.readUInt8(10),
        flags: this._data.readUInt8(11),
        handle: this._data.readUInt32LE(12)
      }
      this.emit('message', `notification: { tick: ${payload.tick}, chip: ${payload.chip}, gpio: ${payload.gpio}, level: ${payload.level}, flags: ${payload.flags}, handle: ${payload.handle} }`)
      this.emit('gpio' + payload.gpio, { value: payload.level !== 0, tick: payload.tick })
      this._data = this._data.slice(this.MESSAGE_SIZE)
    }
  }
}
