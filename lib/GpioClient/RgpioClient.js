...
class RgpioClient extends GpioClient {
  ...
  async listen (mask = 0xFFFFFFFC) {
    ...
    // await this.command(commands.GIL, { longs: [this.gpioChipHandle, 18] })
    // await this.command(commands.GSOX, { longs: [this.gpioChipHandle, 0, 18, 0] })
    // await this.command(commands.GIL, { longs: [this.gpioChipHandle, 18] })
    ...
    for (let gpio = 0; gpio < 32; gpio++) {
      ...
      // await this.command(commands.GIL, { longs: [this.gpioChipHandle, gpio] })
      // await this.command(commands.GSIX, { longs: [this.gpioChipHandle, 0, 18, 0] })
      await this.command(commands.GSOX, { longs: [this.gpioChipHandle, 0, 18, 0] })
      // await this.command(commands.GMODE, { longs: [this.gpioChipHandle, gpio, 2] })
      // await this.command(commands.GR, { longs: [this.gpioChipHandle, gpio] })
      await this.command(commands.GSA, { longs: [this.gpioChipHandle, gpio, this.dataHandle] })
      // await this.command(commands.GIL, { longs: [this.gpioChipHandle, gpio] })
      const result = await this.command(commands.GR, { longs: [this.gpioChipHandle, gpio] })
      this.values[gpio] = result.status
    }
    ...
    for (const gpio in this.values) {
      this.emit('gpio' + gpio, { value: this.values[gpio], tick: 0n })
    }
    // // Get initial map.
  ...
  */
  ...
  ntfDecode (data) {
    /** Emitted when data from the pigpio socket has been received.
      ...
      */
    const payload = {
      tick: data.readBigUInt64LE(0), // nanoseconds since boot?
    ...
    this.emit('gpio' + payload.gpio, { value: payload.level, tick: payload.tick })
