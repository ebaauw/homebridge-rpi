...
class W1Temperature extends ServiceDelegate {
  ...
  async heartbeat (beat) {
    try {
      ...
      if (!this.sensorWarned) {
        this.notFoundBeats += 1
        if (this.rpiAccessory.w1Devices.includes(this.sensorId)) {
      ...
      if (beat % 5 === 0) {
        ...
        this.values.lastUpdated = new Date().toISOString()
  ...
  async read () {
    ...
    if (lines.length < 2 || !lines[0].endsWith(' YES')) {
      throw new Error(
        `${this.sensorId}: 1-wire CRC validation failed (sensor disconnected, noisy bus, or wiring issue)`
      )
    }
