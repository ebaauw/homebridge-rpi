  ...
  constructor (gpioAccessory, params = {}) {
    ...
    if (this.ledId === 0) {
      this.addCharacteristicDelegate({
        ...
        if (value) {
          this.ledChain.colorloop(Math.round(this.values.bri * 2.55)).catch((error) => {
            this.warn(error)
          })
        }
      })
      this.addCharacteristicDelegate({
        ...
        await this.ledChain.update()
        if (value) {
          this.ledChain.cylon(Math.round(this.values.bri * 2.55)).catch((error) => {
            this.warn(error)
          })
        }
      })
  ...
  update (update = true) {
    ...
    this.timer = setTimeout(async () => {
      ...
      const bri = this.values.on
        ? Math.round(this.values.bri * 2.55)
        : 0
