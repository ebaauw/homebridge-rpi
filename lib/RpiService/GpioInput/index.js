...
class GpioInput extends homebridgeLib.ServiceDelegate {
  ...
  constructor (gpioAccessory, params = {}) {
    ...
    this.pi.on('notification', (payload) => {
      ...
      const watchDog = payload.flags === (PigpioClient.notifyFlags.WATCHDOG | this.gpio)
      if (watchDog || newGpioValue !== this._gpioValue) {
        ...
        this.vdebug(
          ...
          this.gpio, watchDog ? 'still ' : '',
          this._gpioValue ? 'high' : 'low', duration
        )
        this.emit('gpio', this._gpioValue, duration, watchDog)
      }
    })
  }
