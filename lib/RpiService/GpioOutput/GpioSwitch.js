...
class GpioSwitch extends GpioOutput {
  constructor (gpioAccessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      setter: async (on) => {
        ...
        {
          const { cmd, params } = this.pi.gpioWrite(this.gpio, value)
          await this.pi.command(cmd, params)
        }
  ...
  async init () {
    if (this.pulse != null) {
      ...
      const { cmd, params } = this.pi.gpioWrite(this.gpio, this.params.reversed ? 1 : 0)
      await this.pi.command(cmd, params)
      // await this.pi.command(
      //   this.pi.commands.WRITE, this.gpio, this.params.reversed ? 1 : 0
      // )
    }
    return super.init()
  }
