...
class RpiAccessory extends AccessoryDelegate {
  static ds18b20SensorIdPattern = /^28-[0-9a-fA-F]{12}$/

  static isDs18b20SensorId (sensorId) {
    return typeof sensorId === 'string' && this.ds18b20SensorIdPattern.test(sensorId)
  }
        ...
        if (this.usesW1) {
          ...
          try {
            ...
            for (const sensorId of localW1Devices) {
              if (RpiAccessory.isDs18b20SensorId(sensorId)) {
  ...
  async addDs18b20 (device) {
    ...
    if (!RpiAccessory.isDs18b20SensorId(device.sensorId)) {
