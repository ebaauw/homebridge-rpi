...
class RpiAccessory extends AccessoryDelegate {
  static DS18B20_SENSOR_ID_REGEX = /^28-[0-9a-fA-F]{12}$/
  ...
  async heartbeat (beat) {
    try {
      ...
      if (this.id === this.platform.localId) { // Local Pi
        ...
        if (this.usesW1) {
          const seenW1Devices = new Set()
          try {
            const localW1Devices = await readdir('/sys/bus/w1/devices')
            for (const sensorId of localW1Devices) {
              if (typeof sensorId === 'string' && RpiAccessory.DS18B20_SENSOR_ID_REGEX.test(sensorId)) {
                if (!seenW1Devices.has(sensorId)) {
                  this.w1Devices.push(sensorId)
                  seenW1Devices.add(sensorId)
                }
              } else {
            ...
            this.warn(
              'local 1-Wire discovery failed; ensure 1-Wire is enabled and /sys/bus/w1/devices is accessible: %s',
              error
            )
          }
        }
        ...
        if (this.hidden && this.powerLedService == null && this.fanService == null && !this.usesW1) {
          await this.pi.command(this.pi.commands.TICK)
        } else {
          ...
          try {
            ...
            if (Array.isArray(remoteState.w1-Devices)) {
              const seenW1Devices = new Set()
              for (const sensorId of remoteState.w1-Devices) {
                if (typeof sensorId === 'string' && RpiAccessory.DS18B20_SENSOR_ID_REGEX.test(sensorId)) {
                  if (!seenW1Devices.has(sensorId)) {
                    this.w1Devices.push(sensorId)
                    seenW1Devices.add(sensorId)
            ...
            this.vdebug('1-Wire: failed to parse remote state for sensor discovery: %s', error)
          }
  ...
  async addDs18b20 (device) {
    ...
    if (device.sensorId == null) {
      if (this.w1BaseName != null) {
        throw new Error(
          'duplicate auto-discovery DS18B20 entry: only one ds18b20 device without a specific sensor ID can be configured per host'
        )
      }
      this.w1BaseName = device.name ?? '1-Wire Temperature Sensor'
      return
    }
    if (typeof device.sensorId !== 'string' || !RpiAccessory.DS18B20_SENSOR_ID_REGEX.test(device.sensorId)) {
