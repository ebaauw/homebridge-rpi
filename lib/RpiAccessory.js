...
class RpiAccessory extends AccessoryDelegate {
  ...
  async heartbeat (beat) {
    try {
      ...
      if (this.id === this.platform.localId) { // Local Pi
        ...
        if (this.usesGpio) {
          try {
            await this.pi.command(this.pi.commands.HWVER)
          } catch (error) {
            this.warn(error)
          }
        }
  ...
  async addLedChain (device) {
    ...
    if (RpiAccessory.LedChainAccessory == null) {
      ...
      GpioLedChain = await import('hb-rpi-tools/GpioLedChain')
    }
    if (device.device === 'p9813') {
      if (GpioLedChain.P9813 == null) {
        await import('hb-rpi-tools/GpioLedChain/P9813')
      }
    } else {
      if (GpioLedChain.Blinkt == null) {
        await import('hb-rpi-tools/GpioLedChain/Blinkt')
      }
    }
