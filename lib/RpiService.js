...
class RpiService extends homebridgeLib.ServiceDelegate {
  ...
  static get GpioStatelessSwitch () { return GpioStatelessSwitch }
...
class GpioStatelessSwitch extends homebridgeLib.ServiceDelegate {
  constructor (rpiAccessory, params = {}) {
    params.name = rpiAccessory.name + ' GPIO' + params.gpio
    params.Service = rpiAccessory.Services.hap.StatelessProgrammableSwitch
    super(rpiAccessory, params)
    this.pi = rpiAccessory.pi
    this.gpio = params.gpio
    this.addCharacteristicDelegate({
  ...
  checkMap (map) {
    if ((map & (1 << this.gpio)) === 0) { // button pressed
      ...
      if (this.pressed != null) {
        const duration = new Date() - this.pressed
