...
class GpioLight extends homebridgeLib.ServiceDelegate {
  constructor (gpioAccessory, params = {}) {
    ...
    this.addCharacteristicDelegate({
      ...
      setter: async (value) => {
        this.values.dutyCycle = value
          ? Math.round(this.values.brightness * 2.55)
          : 0
        await this.update()
      }
    })
    this.addCharacteristicDelegate({
      ...
      unit: '%',
      setter: async (value) => {
        this.values.dutyCycle = Math.round(value * 2.55)
        await this.update()
      }
    })
    this.addCharacteristicDelegate({
  ...
  async heartbeat (beat) {
    try {
      ...
      if (this.values.dutyCycle === 0) {
        this.values.on = false
        this.inHeartbeat = false
        return
      }
      this.values.on = true
      this.values.brightness = Math.round(this.values.dutyCycle / 2.55)
      this.inHeartbeat = false
    } catch (error) {
      this.inHeartbeat = false
