...
class RpiService extends homebridgeLib.ServiceDelegate {
  ...
  checkState (state) {
    ...
    const newBoot = new Date(state.boot)
    if (this.values.boot == null) {
      this.values.boot = newBoot.valueOf()
      this.values.lastBoot = String(newBoot).slice(0, 24)
      this.values.restart = false
      return
    }
    let delta = newBoot.valueOf() - this.values.boot
    if (delta < 0) {
      delta = -delta
    }
    if (delta > 5000) {
    ...
    this.values.restart = delta > 5000
  }
}
