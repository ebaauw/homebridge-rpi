...
class PigpioLedChain extends events.EventEmitter {
  ...
  async update () {
    ...
    for (let id = 0; id < this._nLeds; id++) {
      ...
      /** Emitted when client has disconnected from the pigpio socket.
        ...
        * @param {int} id - The LED id, between 0 and nLeds.
        * @param {int[]} led - Four bytes of data.
        */
      this.emit('led', id, led)
  ...
  async _setPin (gpio, value) {
    ...
    return new Promise((resolve, reject) => {
      this._pi.emit('command', PigpioClient.commands.WRITE, gpio, value, '')
      this._buffer.writeUInt32LE(gpio, 4)
      this._buffer.writeUInt32LE(value, 8)
      this._pi.emit('request', this._buffer)
      this._pi._cmdSocket.write(this._buffer, () => {
        resolve()
      })
    })
  }
