...
class PigpioClient extends events.EventEmitter {
  ...
  /** Create a new PigpioClient instance.
    ...
    * @param {string} [params.hostname='localhost'] - Hostname of the pigpio server.
    * @param {number} [params.port=8888] - Port of the pigpio server.
    */
  constructor (params = {}) {
    super()
    this._debug = debug('PigpioClient' + ++id)
  ...
  /** Open a socket to pigpio for sending commands and receiving responses.
    */
  async connect () {
    this._debug('connect()')
    this._cmdSocket = net.createConnection(this._params.port, this._params.hostname)
    this._cmdSocket.on('data', this._onCmdData.bind(this))
    this._cmdSocket.on('error', async (error) => {
      // FIXME: I see two error messages when connect fails?!
  ...
  /** Disconnect from pigpio.
    */
  async disconnect (errorOccurred = false) {
    this._debug('disconnect(%j)', errorOccurred)
    this._cmd = null
    if (this._dataHandle != null && !errorOccurred) {
      await this.command(commands.NC, this._dataHandle)
      delete this._dataHandle
    }
    ...
    this._debug('disconnect(%j) => ok', errorOccurred)
  }
  ...
  _onCmdData (data) {
    ...
    // Check if response to last command sent.
    ...
    // Return result for commands that always succeed.
    ...
    if (res >= 0 && extCommands.includes(cmd)) {
      if (data.length >= 16 + res) {
        this.emit(cmd, res, data.slice(16))
        return
      }
      this._data = data
      return
  ...
  async command (cmd, p1 = 0, p2 = 0, p3 = Buffer.from('')) {
    while (this._cmd != null) {
      this._debug(
        'command(%s): wait for previous %s command to finish',
        commandName(cmd), commandName(this._cmd)
      )
      await homebridgeLib.timeout(200)
    }
    this._debug('command(%s, %j, %j, %j)', commandName(cmd), p1, p2, p3.toString())
    this._cmd = cmd
    this._p1 = p1
    this._p2 = p2
    if (this._cmdSocket == null) {
    ...
    // this._debug('command(%s): send request: %j', commandName(cmd), request)
    ...
    if (p[1] != null) {
      const result = p[1].toString()
      this._debug('command(%s) => %s', commandName(cmd), result)
      return result
    }
    this._debug('command(%s) => %d', commandName(cmd), status)
    return status
  }

  /** Sends a shell command to pigpio.
  ...
  async shell (script, params = '') {
    const status = await this.command(
      commands.SHELL, script.length, 0, Buffer.concat(
  ...
  async readFile (filename) {
    ...
    let result
    do {
      result = await this.command(commands.FR, handle, this._params.blockSize)
      s += result.toString()
  ...
  /** Listen to notifications for changed GPIO values.
    *
    * Opens a second socket to pigpio to receive notifications.
    * @param {int} mask - Bitmask of the GPIO pins to monitor.
    */
  async listen (mask = 0x0ffffffc) {
    this._debug('listen(0x%s)', mask.toString(16))

    // Open data socket.
    this._dataSocket = net.createConnection(this._params.port, this._params.hostname)
    this._dataSocket.on('data', this._onDataData.bind(this))
    this._dataSocket.on('error', (error) => { this.warn(error) })
    ...
    this._debug('command(%s): send data request: %j', commandName(commands.NOIB), request)
