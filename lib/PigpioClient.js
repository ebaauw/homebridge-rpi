...
class PigpioClient extends events.EventEmitter {
  ...
  /** Create a new PigpioClient instance.
    ...
    * @param {string} [params.hostname='localhost'] - Hostname of the pigpio
    * socket.
  ...
  async connect () {
    ...
    this._cmdSocket.on('error', () => { this._disconnect() })
    this._cmdSocket.on('close', () => { this._disconnect() })
  ...
  async command (cmd, p1 = 0, p2 = 0, p3 = '') {
    ...
    /** Emitted when sending a command to the pigpio socket.
      ...
      * @param {string} p3 - The command's third, extended parameter.
      */
    this.emit('command', cmd, p1, p2, p3)
    ...
    if (p[1] != null) {
      ...
      /** Emitted when receving a command response from the pigpio socket.
        ...
        * @param {?string} result - The command extended result.
        */
      this.emit('response', cmd, status, result)
      return result
    }
  ...
  async readFile (filename) {
    const handle = await this.command(commands.FO, 1, 0, filename)
    if (handle !== 0) {
      this.emit('warning', new Error('file handle !== 0'))
    }
    let s = ''
    let result
    do {
  ...
  /** Subscribe to notifications for changed GPIO values.
    ...
    * @param {int} [mask=0x0ffffffc] - Bitmask of the GPIOs to monitor.
    * @emits listen
    * @throws `Error` - When connection fails.
    */
  async listen (mask = 0x0ffffffc) {
    ...
    this._dataSocket.on('error', () => { this._disconnect() })
    this._dataSocket.on('close', () => {
      // delete this._dataSocket
      this._disconnect()
    })
  ...
  _onDataData (data) {
    ...
    while (data.length >= 12) {
      ...
      /** Emitted when a GPIO notification is received from pigpio.
        ...
        * @param {int} map - A bitmap of the GPIO values.
        */
      this.emit('notification', map)
      data = data.slice(12)
    }
