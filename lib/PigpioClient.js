...
class PigpioClient extends events.EventEmitter {
  ...
  /** Make a command connection to the pigpio socket
    ...
    */
  async connect () {
    this._cmdSocket = net.createConnection(this._params.port, this._params._hostname)
    this._cmdSocket
  ...
  async listen (mask = 0x0ffffffc) {
    ...
    // Open data socket.
    this._dataSocket = net.createConnection(this._params.port, this._params._hostname)
    this._dataSocket
  ...
  _onDataData (data) {
    ...
    while (data.length >= 12) {
      ...
      /** Emitted when a GPIO notification is received from pigpio.
        ...
        * @param {int} tick - Timestamp in µs (wraps at 2^32).
        */
      this.emit('notification', map, tick)
      data = data.slice(12)
    }
  }
}
